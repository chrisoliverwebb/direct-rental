...
export function ContactCreateDialog({
  ...
  const handleImport = async () => {
    ...
    if (result.status === "COMPLETED") {
      toast.success(`Imported ${result.importedRows} contacts successfully`);
      onOpenChange(false);
      return;
    }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Add a single contact or import a CSV batch.</DialogDescription>
          </DialogHeader>
          <DialogBody className="grid gap-6">
            ...
            <form className="grid gap-4" onSubmit={form.handleSubmit(handleCreateContact)}>
              ...
              <div className="flex items-center justify-end gap-2">
                <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={createContact.isPending}>
                  Cancel
                </Button>
                ...
                <div className="grid gap-3 rounded-lg border p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Showing {previewRows.length} of {previewTotal} contacts to be imported
                    </p>
                  </div>
                  <Table>
                    <TableHeader>
